rule amazon_s3_download_datasets:
    ...
    benchmark:
        out = "logs/benchmark/data/download/s3/amazon_s3_download_datasets.tsv"
    run:
        ...
        cmd = f"mkdir -p {output.storage_path} ; {params.s3cmd} -c {input.config} get --recursive s3://{params.dataset}/ {output.storage_path} ; "
