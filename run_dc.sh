...
~/.conda/envs/snakemake/bin/snakemake -s workflow/rules/data/concat/entry_point.smk --cluster 'sbatch --cpus-per-task={threads} --mem={resources.mem_mb}M -p {params.queue}' --jobs 20 --latency-wait 50 --rerun-incomplete
