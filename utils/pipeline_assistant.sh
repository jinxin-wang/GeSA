...
function prepare_download_from_ega {
    ...
    echo -e "${OKGREEN}[info]${ENDC} setup EGA download configuration file "
    DATABASE=${EGA} ;
    echo -e "${WARNING}[check point]${ENDC} please set the username [EGA account]: "
    read ega_username
    echo -e "${WARNING}[check point]${ENDC} please set the password [EGA account]: "
    read ega_password
    echo -e "{\"username\":\"${ega_username}\",\"password\":\"${ega_password}\"}" > ${WORKING_DIR}/config/ega_config.json
    echo -e "${WARNING}[check point]${ENDC} Do you need to check the list of your datasets: [y]/n ? "
    read line
    ...
    echo -e "${WARNING}[check point]${ENDC} please provide the datasets one by one, entre to continue"
    datasets_array=()
    while true ; do
	read line
	if [ -z ${line} ] ; then
    ...
    echo -e "${WARNING}[check point]${ENDC} Please set the number of connections, default: 100"
    read line
    if [ -z ${line} ] ; then
	conn=100
    else
...
function prepare_download_from_amazon_s3 {
    ...
    echo -e "${OKGREEN}[info]${ENDC} setup Amazon S3 download configuration file "
    DATABASE=${AMAZON} ;

    # access_key = AKIATBHNOLFVJG6VRUOG
    echo -e "${WARNING}[check point]${ENDC} Please provide the access key: [for example: AKIATBHNOLFVJG6VRUOG]"
    read access_key

    # secret_key = QkD5aMvi4MNsqlRy2yb03zGIDss9rxjjZaDcVkjx
    echo -e "${WARNING}[check point]${ENDC} Please provide the secret key: [for example: QkD5aMvi4MNsqlRy2yb03zGIDss9rxjjZaDcVkjx]"
    read secret_key
    
    # host_base  = s3-eu-west-3.amazonaws.com
    echo -e "${WARNING}[check point]${ENDC} Please provide the region: [for example: eu-west-3]"
    read region
    
    host_base="s3-${region}.amazonaws.com"
    host_bucket="%(bucket)s.${host_base}"

    echo -e "${WARNING}[check point]${ENDC} Please provide the path on amazon s3: [for example: s3://homlsxyr-598731762349/F23A430001132-04_HOMlsxyR ] "
    read s3_path

    echo -e "access_key = ${access_key} \nsecret_key = ${secret_key} \nhost_base = ${host_base} \nhost_bucket = ${host_bucket} \n" > ${WORKING_DIR}/config/s3_config.yaml
    
    echo -e "${OKGREEN}[info]${ENDC} checking total data size to download. "
    ...
    echo -e "${OKGREEN}[info]${ENDC} total data size: ${dataset_size} "
...
function prepare_variant_call_table {
	...
	echo -e "${OKGREEN}[info]${ENDC} variant call table is ready: "
	cat ${VARIANT_CALL_TABLE} ;
	
    elif [ -f ${VAR_TABLE} ] ; then
	echo -e "${OKGREEN}[info]${ENDC} copy the variant call table to current directory"
	cp ${VAR_TABLE} ${VARIANT_CALL_TABLE} ;
	
    elif [ ${MODE} == ${TvN} ] || [ ${MODE} == ${TvNp} ]; then
	echo -e "${OKGREEN}[info]${ENDC} Generating variant call table..."
	cd ${STORAGE_DIR} ;
	echo -e "${OKGREEN}[info]${ENDC} samples:  " ;
	ls ;
	nsamples=(*_N) ;
	tsamples=(*_T) ;
	cd - ;
	...
	echo -e "${OKGREEN}[info]${ENDC} variant call table is done: "
	cat ${VARIANT_CALL_TABLE} ;
	...
	cat variant_call_list_${T}.tsv ;
	    ...
	    echo -e "${OKGREEN}[info]${ENDC} init empty variant call table..." ;
...
if [ ${DO_DOWNLOAD} == true ] ; then
    ...
    if [ ${INTERACT} == true ] && [ ! -f ${WORKING_DIR}/config/download.yaml ] ; then
        # echo -e "${WARNING}[check point]${ENDC} Please set the download configuration file:  [enter to continue or set the path]" ;
        # read line ;

        # if [ ! -z ${line} ] && [ -f ${line} ] ; then
        #     echo -e "${OKGREEN}[info]${ENDC} copy your download configuration file to config/ "
        #     cp ${line} ${WORKING_DIR}/config/download.yaml ;
        #     echo -e "${OKGREEN}[info]${ENDC} ${WORKING_DIR}/config/download.yaml : "
        #     cat ${WORKING_DIR}/config/download.yaml
        #     echo -e "${OKGREEN}[info]${ENDC} end of the configuration file "

        # else
            echo -e -e "${WARNING}[check point]${ENDC} please choose the database by the number : \n  1. EGA \n  2. iRODS \n  3. Amazon S3 \n  4. BGI GeneAn \n  5. FTP Server \n  6. From backup storage "
            read line
...
####         verify md5sum code      ####
#########################################
if [ ${DO_MD5SUM} == true ] && [ ${DATABASE} != ${IRODS} ] ; then
...
if [ ${DATA_FILETYPE} == ${FASTQ} ] && ( [ ${DO_DOWNLOAD} == true ] || [ ${DO_CONCAT} == true ] ) ; then
    ...
    if [ -z "${STORAGE_DIR}" ] ; then
        if [ ${INTERACT} == true ] ; then
            ...
            echo -e "${WARNING}[check point]${ENDC} Please provide the directory of raw data. [enter to continue with default path: ${OKGREEN}${DEFAULT_STORAGE_DIR}${ENDC} ] "
            read line
            if [ -z ${line} ] ; then
    ...
    if [ -z "${CONCAT_DIR}" ] ; then
        if [ ${INTERACT} == true ] ; then
            ...
            echo -e "${WARNING}[check point]${ENDC} Please provide the directory for the concatenation of raw data. [enter to continue with default path: ${OKGREEN}${DEFAULT_CONCAT_DIR}${ENDC} ] "
            read line
            if [ -z ${line} ] ; then
    ...
    if [ ${INTERACT} == true ] ; then 
    	echo -e "${WARNING}[check point]${ENDC} Please provide the table of samples with information about the pairs of reads if it is possible "
    	...
    	if [ ! -z ${line} ] ; then
    	    ...
    	    echo -e "${WARNING}[check point]${ENDC} Please provide the variant call table : [enter to continue and create the table later or set path now] "
    	    read line ;
    	    if [ ! -z ${line} ] && [ -f ${line} ] ; then
...
if [ ${DO_PIPELINE} == true ] ; then 
    ...
    if  [ ${DATA_FILETYPE} == ${FASTQ} ] ; then 
        ...
        build_pipeline_cmd "${CONFIG_OPTIONS} sam2fastq=True " ${} ${FASTQ} ${RUN_PIPELINE_SCRIPT} ;
    fi
