...
if ( [ -d DNA_samples ] || [ -d bam ] ) && [ -f ${VAR_TABLE} ] && [ -f workflow ] ; then
    echo "Starting pipeline" ; 
    /mnt/beegfs/userdata/j_wang/.conda/envs/snakemake/bin/snakemake --cluster 'sbatch --cpus-per-task={threads} --mem={resources.mem_mb}M -p {params.queue}' --jobs 20 --latency-wait 50 --rerun-incomplete --config samples=${SAMPLES} seq_type=${SEQ_TYPE} mode=${MODE} ;
